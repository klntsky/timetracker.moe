...
export function entriesReducer(state: EntriesState, action: EntriesAction): EntriesState {
  return match(action)
    ...
    .with(
      { type: 'UPDATE_ENTRY' },
      ({ entryId, updates }: { entryId: number; updates: Partial<TimeEntry> }) => ({
        ...state,
    ...
    .with(
      { type: 'UPDATE_ENTRY_DURATION' },
      ({ entryId, additionalDuration }: { entryId: number; additionalDuration: number }) => ({
        ...state,
        entries: state.entries.map((e) =>
          e.id === entryId ? { ...e, duration: e.duration + additionalDuration } : e
        ),
      })
    )
    .with(
      { type: 'CHANGE_ENTRY_PROJECT' },
      ({ entryId, projectId }: { entryId: number; projectId: number }) => ({
        ...state,
        entries: state.entries.map((e) => (e.id === entryId ? { ...e, projectId } : e)),
      })
    )
